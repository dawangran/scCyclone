...
def rank_apas_groups_df(
    ...
    key (str): Key for the data.
    pval_cutoff (float): P-value cutoff for analysis.
    min_dpui (float): Minimum difference value.
    max_dpui (float): Maximum difference value.
    dpr_cutoff (float): dpr cutoff for analysis.
    tpr_cutoff (float): tpr cutoff for analysis.
    rpr_cutoff (float): rpr cutoff for analysis.
    compare_abs (bool): Flag to compare absolute values.
    ...
    colnames = ["names", "dpui", "pvals", "pvals_adj", "dpr","rpr","tpr","rpui","tpui","gene_name"]
    ...
    d = d[(abs(d["dpui"]) >= min_dpui if compare_abs==True else d["dpui"] >= min_dpui) & 
          (abs(d["dpui"]) <= max_dpui if compare_abs==True else d["dpui"] <= max_dpui)]
